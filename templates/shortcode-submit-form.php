...
function wp_fundraising_campaign_form_shortcode( $atts ){
    if(isset($_GET['status']) && $_GET['status']=='success'){
        ?>
            <div class="row campaign-submit-success">
                <div class="offset-md-3 col-md-6 text-center">
                    <p><?php esc_html_e('You\'ve created campaign successfully!','wp_fundraising');?></p>
                    <a class="btn btn-primary" href="<?php echo esc_url(home_url('/'));?>wf-campaign-form/"><?php esc_html_e('Create Another One','wp_fundraising')?></a>
                    <a class="btn btn-primary" href="<?php echo esc_url(home_url('/'));?>"><?php esc_html_e('Go back to home','wp_fundraising');?></a>
                </div>
            </div>
        <?php
    }else{
        ...
        if ( is_user_logged_in() ) {
            ...
            $html .= '<form method="post" action="?status=success" id="campaign_form" class="xs-campaign" enctype="multipart/form-data">';
